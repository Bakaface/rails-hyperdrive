...
RSpec.describe Rails::Generators::RailsBoost::InstallGenerator do
  ...
  it "writes .mcp.json with the mount path" do
    ...
    expect(body).to include("/_boost/mcp")
  end
  ...
  it "is idempotent — re-running does not duplicate the mount" do
    ...
    run_generator(["--yes"])
    routes = File.read(File.join(@app_dir, "config/routes.rb"))
    expect(routes.scan("Rails::Boost::Engine").length).to eq(1)
  end
  ...
  it "honors --dry-run by writing no files" do
    ...
    expect(File.read(File.join(@app_dir, "config/routes.rb"))).not_to include("Rails::Boost::Engine")
  end
  ...
  it "honors --mount-at and writes the initializer when non-default" do
    run_generator(["--yes", "--mount-at", "/admin/boost"])
    body = File.read(File.join(@app_dir, ".mcp.json"))
    expect(body).to include("/admin/boost/mcp")
    routes = File.read(File.join(@app_dir, "config/routes.rb"))
    expect(routes).to include("/admin/boost")
    initializer = File.join(@app_dir, "config/initializers/rails_boost.rb")
    expect(File).to exist(initializer)
    expect(File.read(initializer)).to include('c.mount_at = "/admin/boost"')
  end

  it "does NOT write the initializer when --mount-at is the default" do
    run_generator(["--yes"])
    expect(File).not_to exist(File.join(@app_dir, "config/initializers/rails_boost.rb"))
  end
  ...
  it "normalizes --mount-at without a leading slash" do
    run_generator(["--yes", "--mount-at", "boost"])
    body = File.read(File.join(@app_dir, ".mcp.json"))
    expect(body).to include("/boost/mcp")
  end

  it "strips a trailing slash from --mount-at" do
    run_generator(["--yes", "--mount-at", "/_boost/"])
    body = File.read(File.join(@app_dir, ".mcp.json"))
    expect(body).to include("/_boost/mcp")
    expect(body).not_to include("/_boost//mcp")
  end
end
