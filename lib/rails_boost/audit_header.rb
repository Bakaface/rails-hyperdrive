...
module Rails
  module Boost
    ...
    #
    #   # rails_boost: source=sidekiq@7.3.4
    #   # rails_boost: sha256=ab12cd34...
    #   # rails_boost: installed_at=2026-05-21T15:42:11Z
    #
    module AuditHeader
      module_function
      ...
      def build(source_gem:, version:, body:, installed_at: Time.now.utc)
        ...
        [
          "# rails_boost: source=#{source_gem}@#{version}",
          "# rails_boost: sha256=#{sha}",
          "# rails_boost: installed_at=#{installed_at.iso8601}"
        ].join("\n")
      end
