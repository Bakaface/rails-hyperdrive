...
module Rails
  module Boost
    ...
    # technology stack. Shared between the `boost:init` generator and the
    # `describe_app` MCP tool / `boost://stack-profile` resource so the installer
    ...
    class StackProfile
      ...
      def gem_skills_info
        ...
        ::Rails::Boost::SkillDiscovery.discover.map do |skill|
          {
            name: skill.name,
            gem: skill.gem,
