      ...
      class InstallGenerator < ::Rails::Generators::Base
        ...
        def verify_environment
          unless defined?(::Rails) && ::Rails.respond_to?(:root) && ::Rails.root
            ...
            raise Thor::Error, "rails_boost: not in a Rails app"
          end
          unless ::Rails.respond_to?(:env) && ::Rails.env.development?
            env = ::Rails.respond_to?(:env) ? ::Rails.env.to_s : "unknown"
            warn "rails_boost: boost:init must run with Rails.env=development (current: #{env})"
            raise Thor::Error, "rails_boost: refuse to run outside development (Rails.env=#{env})"
          end
        ...
        def parse_stack_profile
          @stack_profile = ::Rails::Boost::StackProfile.from_lockfile(
        ...
        def prompt_for_arch_skills
          ...
          raise Thor::Error, "rails_boost: aborted by user"
        end
        ...
        def discover_gem_skills
          @discovered_gem_skills =
            if options[:skip_skills]
              ...
              ::Rails::Boost::SkillDiscovery.discover
            end
        end
        ...
        def write_arch_skills
          ...
          @selected_arch_skills.each do |name|
            src = File.expand_path("../../../rails_boost/skills/#{name}/SKILL.md", __dir__)
        ...
        def write_gem_skills
          ...
          @discovered_gem_skills.each do |skill|
            audit = ::Rails::Boost::AuditHeader.build(
              source_gem: skill.gem,
              version: skill.spec_version,
              body: skill.body
            )
            body = ::Rails::Boost::AuditHeader.inject_into_frontmatter(skill.body, audit)
        ...
        def write_initializer
          ...
          template "initializer.rb.tt", "config/initializers/rails_boost.rb"
        end
        ...
        def mount_engine
          ...
          snippet = "  mount Rails::Boost::Engine => \"#{mount_path}\" if Rails.env.development?\n"
          inject_into_file routes_file, snippet, after: /Rails\.application\.routes\.draw do\s*\n/
        end

        def print_summary
          say ""
          say_status :done, "rails_boost initialized", :green
          say ""
          say "  Files:"
          say "    - .mcp.json"
          ...
          say "    - config/initializers/rails_boost.rb" if mount_path != DEFAULT_MOUNT_AT
        ...
        no_tasks do
          ...
          #   - prepend `/` if missing
          #   - strip any trailing `/` so `/_boost/` and `/_boost` are the same
          def mount_path
